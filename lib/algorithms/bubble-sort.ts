...
export function* bubbleSortSteps(input: number[]): Generator<SortStep> {
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    for (let j = 0; j < n - i - 1; j++) {
      comparisons++
      yield { bars: [...arr], states: makeStates(j, 'comparing', n - i), description: `Comparing index ${j} (${arr[j]}) and ${j + 1} (${arr[j + 1]})`, comparisons, swaps }

      if (arr[j] > arr[j + 1]) {
        ;[arr[j], arr[j + 1]] = [arr[j + 1], arr[j]]
        swapped = true
        swaps++
        yield { bars: [...arr], states: makeStates(j, 'swapping', n - i), description: `Swapped: ${arr[j + 1]} and ${arr[j]}`, comparisons, swaps }
      }
    }
    if (!swapped) break
  }
