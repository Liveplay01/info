...
export function* timSortSteps(input: number[]): Generator<SortStep> {
  ...
  function insertionSortRange(left: number, right: number) {
    for (let i = left + 1; i <= right; i++) {
      const key = arr[i]
      let j = i - 1
      steps.push({ bars: [...arr], states: makeStates([left, right], [i], 'comparing'), description: `Insertion sort: inserting ${key} in run [${left}..${right}]`, comparisons, swaps })
      while (j >= left && arr[j] > key) {
        arr[j + 1] = arr[j]
        swaps++
        comparisons++
        steps.push({ bars: [...arr], states: makeStates([left, right], [j, j + 1], 'swapping'), description: `Shifting ${arr[j]} right`, comparisons, swaps })
        j--
      }
      arr[j + 1] = key
    }
  ...
  function mergeRanges(l: number, m: number, r: number) {
    ...
    steps.push({ bars: [...arr], states: arr.map((_, idx) => (idx >= l && idx <= r ? 'pivot' : 'default')), description: `Merging runs [${l}..${m}] and [${m + 1}..${r}]`, comparisons, swaps })
    ...
    steps.push({ bars: [...arr], states: arr.map((_, idx) => (idx >= l && idx <= r ? 'sorted' : 'default')), description: `Merged run [${l}..${r}]`, comparisons, swaps })
  }

  // Sort runs
  for (let i = 0; i < n; i += MIN_MERGE) {
    const right = Math.min(i + MIN_MERGE - 1, n - 1)
    steps.push({ bars: [...arr], states: arr.map((_, idx) => (idx >= i && idx <= right ? 'pivot' : 'default')), description: `Sorting run [${i}..${right}] with Insertion Sort`, comparisons, swaps })
